...
def cargar_inicializacion_SVD():
    data_user = cargarDatos.cargar_ratings_limpios()
...
def obtener_la_base_datos_actual_usuario(id_usuario):
    #data_user = cargarDatos.cargar_ratings_limpios()
    data_user = cargarDatos.csv_ratings_limpios.copy()
...
def anadir_nueva_pelicula(item_id):
    ...
    print("para aqui 2")
    if isinstance(modelSVD.train_set.item_ids, list):
        print("es lista")
        modelSVD.train_set.item_ids.append(item_id)
    else:
...
def filtro_colaborativo(id_usuario):
    ...
    if id_user not in modelSVD.train_set.uid_map:
        return recomendaciones_usuario_nuevo(id_user)
    else:
        return recomendaciones_SVD(id_user)
