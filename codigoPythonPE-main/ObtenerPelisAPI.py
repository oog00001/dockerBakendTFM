...
imgDir = os.path.join(data_dir, "img_movies")

movies_dir_o = os.path.join(data_dir, "movies_dataset.csv")
...
def getCsv():
    ...
    dfMovies.to_csv(csv_path, index=False)
...
def cargarVariablesLocaLesYActualizar():
    movies_source = os.getenv("MOVIES_SOURCE", "local").lower()
    use_api = movies_source == "api" or not os.path.exists(imgDir)
    ...
    if use_api:
        ...
        dfMovies = pd.read_csv(movies_dir_o)
        dfMovies['genre_ids'] = dfMovies['genre_ids'].apply(convert_genre_ids)

    # Este bloque es común para ambos casos
    app.mount("/img_movies", StaticFiles(directory=imgDir), name="img_movies")
